...
// تولید رشته JSON ساده از map (برای پاسخ GET)
string map_to_json(const map<string, string>& data) {
    string json = "{";
    for (const auto& pair : data) {
        json += "\"" + pair.first + "\": \"" + pair.second + "\",";
    }
    if (json.length() > 1) { // حذف کامای اضافی
        json.pop_back();
    }
    json += "}";
    return json;
...
// پارس کردن JSON ساده (برای درخواست POST)
map<string, string> parse_simple_json(const string& json_str) {
    ...
    while (getline(ss, segment, ',')) {
        ...
        if (colon != string::npos) {
            ...
            // حذف فاصله و " از اطراف Key و Value
            ...
            // --- مسیر API: POST /api/users ---
                ...
                try {
                    ...
                    if (new_user_data.count("name") && new_user_data.count("email")) {
                        ...
                        // پاسخ 201 Created در RESTful API
