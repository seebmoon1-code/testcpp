...
#include <vector>
#include <memory> 
#include <shared_mutex> // NEW: اضافه شدن هدر shared_mutex برای کارایی بهتر
...
// تغییر مهم: استفاده از Shared Mutex برای کارایی خواندن‌های موازی (Read/Write Lock)
...
// --- تابع هندل کردن آپلود فایل (ایمنی OOM: Stream-to-Disk) ---
void handle_upload_stream(int client_socket, const string& initial_body, long content_length) {
    ...
    while (remaining_to_read > 0) {
        ...
        if (bytes_read <= 0) {
            ...
            outfile.close();
            // فایل ناقص را حذف می‌کنیم (مهم برای جلوگیری از فایل‌های آسیب‌دیده)
    ...
    do { 
        ...
        if (method == "GET") {
            ...
            if (path == "/api/users") {
                ...
                // حفاظت از خواندن: shared_lock اجازه خواندن موازی را می‌دهد
                {
            ...
            if (path == "/upload") {
                if (headers.count("content-length")) {
                    try {
                        long content_length = stol(headers["content-length"]); 
                        // فراخوانی تابع Stream-to-Disk (ایمنی OOM)
                ...
                try {
                    ...
                    if (new_user_data.count("name") && new_user_data.count("email")) {
                        ...
                        // حفاظت از نوشتن: unique_lock انحصار نوشتن را تضمین می‌کند
                        {
