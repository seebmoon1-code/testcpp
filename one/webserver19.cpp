...
void handle_upload_stream(int client_socket, const string& initial_body, long content_length) {
    ...
    // NOTE: For a real server, you should ensure the filename is safe and unique (e.g., using UUIDs)
    ...
    while (remaining_to_read > 0) {
        ...
        // استفاده از read به جای recv برای سازگاری بهتر با Unix/Linux
...
void handle_client(int client_socket, Router& router) {
    ...
    // یک حلقه برای مدیریت Keep-Alive
    do { 
        // در هر تکرار، بافر را پاک می‌کنیم
        fill(buffer.begin(), buffer.end(), 0);
        valread = read(client_socket, buffer.data(), BUFFER_SIZE - 1); 
        
        if (valread <= 0) {
            // Timeout یا بسته شدن اتصال
            break; 
        }
        ...
        if (headers.count("content-length")) {
            try {
                ...
                // نادیده گرفتن در صورت خطا در تبدیل
            }
        }

        // اگر طول بدنه در اولین read کمتر از Content-Length باشد، ادامه خواندن
        if (body.length() < content_length) {
            ...
            while (remaining > 0) {
                ...
                // مطمئن می‌شویم که بافر را کاملاً پاک کرده‌ایم
                fill(buffer.begin(), buffer.end(), 0);
                long bytes_read = read(client_socket, buffer.data(), read_now);
        ...
        // مسیریابی درخواست و گرفتن پاسخ
        string response_str = router.route_request(method, path, headers, body, client_socket);
        
        // اگر تابع روتر به جای پاسخ، "SERVED" برگرداند (برای فایل‌های استاتیک و آپلود جریانی)، یعنی پاسخ از داخل تابع ارسال شده و نباید دوباره ارسال شود.
        if (response_str != "SERVED") {
            send(client_socket, response_str.c_str(), response_str.length(), 0);
        }
        
        // بررسی هدر Connection برای Keep-Alive
...
void setup_routes(Router& router) {
    // API Routes (JSON)
    router.register_route("GET", "/api/users", api_users_get_handler);    // Read All
    router.register_route("POST", "/api/users", api_users_post_handler);  // Create New
    router.register_route("PUT", "/api/users/", api_users_put_handler);   // Update (Wildcard)

    // Other API/Utility Routes
    router.register_route("GET", "/count", count_get_handler);             // Atomic Counter Test
    router.register_route("GET", "/files", files_get_handler);             // File Manager UI (HTML)
    router.register_route("POST", "/upload", upload_post_handler);          // File Upload Handler
    router.register_route("DELETE", "/files/", files_delete_handler);       // File Delete Handler (Wildcard)
...
int main() {
    ...
    // تنظیمات زبان برای زمان (اختیاری)
    ...
    // ۳. آماده‌سازی سوکت سرور
    int server_fd, new_socket;
    struct sockaddr_in address;
    int addrlen = sizeof(address);
    
    // ساخت فایل دیسکریپتور سوکت
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        perror("خطا در ساخت سوکت");
        return 1;
    }

    // تنظیمات سوکت برای استفاده مجدد از آدرس (اختیاری، اما توصیه می‌شود)
    int opt = 1;
    ...
    address.sin_addr.s_addr = INADDR_ANY; // گوش دادن به همه رابط‌ها
    ...
    // ۴. حلقه اصلی پذیرش اتصال‌ها (Accept Loop)
    while(true) {
        log_message("منتظر اتصال جدید...");
        
        if ((new_socket = accept(server_fd, (struct sockaddr *)&address, (socklen_t*)&addrlen)) < 0) {
            // در صورت خطا در accept، می‌توانیم با استفاده از errno تشخیص دهیم که آیا خطای قابل چشم‌پوشی است یا نه.
            // اما در حالت عادی برای سادگی، ادامه می‌دهیم.
            if (errno == EINTR) continue; // اگر توسط سیگنال قطع شد، ادامه بده
            perror("خطا در accept");
            continue;
        }
        
        // ایجاد یک نخ (Thread) جدید برای مدیریت کلاینت (برای پشتیبانی از Multi-Thread)
        thread client_thread(handle_client, new_socket, ref(router));
        client_thread.detach(); // جدا کردن نخ از حلقه اصلی
    }

    // این بخش unreachable است اما برای تکمیل کد اضافه می‌شود
    close(server_fd);
